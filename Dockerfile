...
RUN CGO_ENABLED=0 go install -ldflags="-X github.com/segmentio/ctlstore/pkg/version.version=$VERSION" ./pkg/cmd/ctlstore \
  && cp ${GOPATH}/bin/ctlstore /usr/local/bin

RUN CGO_ENABLED=0 go install -ldflags="-X github.com/segmentio/ctlstore/pkg/version.version=$VERSION" ./pkg/cmd/ctlstore-cli \
